...
int event_init() {
    ...
    try{
        custom_event_map.create(1024);
    }catch (bsl::NullPointerException & e){
        //log_fatal("create hashmap error:%s",e.what());
        return -1;
    }
    return 0;
}
...
/*event_t* event_new(event_socket_t fd) {
    return (event_t*) bufferevent_socket_new(base, fd, BEV_OPT_CLOSE_ON_FREE | BEV_OPT_DEFER_CALLBACKS | BEV_OPT_THREADSAFE);
}*/

//used for thread workers
event_t * event_new(event_socket_t fd, event_base *evbase) {
    return (event_t*) bufferevent_socket_new(evbase, fd, BEV_OPT_CLOSE_ON_FREE | BEV_OPT_DEFER_CALLBACKS );//| BEV_OPT_THREADSAFE);
}
...
event_timer_t* event_timer_new(timer_cb_t cb, unsigned long interval_usec, bool repeating, void *ctx, event_base *base) {
    event_timer_t *evt = (event_timer_t*) malloc(sizeof(event_timer_t));
    evt->cb = cb;
    evt->ctx = ctx;
    struct event* ev = event_new(base, -1, (repeating? EV_PERSIST : EV_TIMEOUT), my_timer_cb, evt);
