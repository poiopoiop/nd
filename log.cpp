...
#define LOG_BUF_SIZE (64*1024*1024)
...
static char _log_temp_buf[64*1024];
...
void* log_thread_loop(void*) {
    ...
    while(true) {
        ...
        tv.tv_usec = 0;
        select(log_pipe[0]+1, &rfds, NULL, NULL, &tv);
...
int log_init() {
    ...
    memset(logwf_buf, 0, LOG_BUF_SIZE);
    ...
    rc = fcntl(log_pipe[1], F_SETFL, O_NONBLOCK);
    if (-1 == rc) {
        perror("Error fcntl log pipe");
        exit(1);
    }

    //init the mutex for thread
    pthread_mutex_t blank_mutex = PTHREAD_MUTEX_INITIALIZER;
    memcpy(&log_mutex, &blank_mutex, sizeof(blank_mutex));
